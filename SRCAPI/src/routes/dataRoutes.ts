...
router.get('/fetchContent', async (req: Request, res: Response) => {
	try {
		...
		console.log(`✅ "${content.length.toString()}" elements trouve pour la page "${page.toString()}" et le nom "${name}"`)
		res.json(content);
	} catch (err) {
...
router.get('/downloadUrl', (req: Request, res: Response) => {
	try {
		...
		if (id && passKey && passKey.length > 10) {
			...
			console.log(`✅ url trouvee : "${result.url}"`)
		} else {
			console.error('❌ ID ou passkey invalide ou manquante');
		}
		res.json(result);
	} catch (err) {
